...
const WEBHOOK_URL = "https://discord.com/api/webhooks/1441654006110883930/n3sl4lqQrRtGclQ2BCuBYcYB8q3GN448D_n0sfimrOSo-1MFPYVGpZ9BKxcOgYKv1M9s"; 
...
app.post('/api/verify-token', (req, res) => {
  ...
  if (token === '36') res.json({ ok: true });
  else res.status(401).json({ ok: false });
});

app.post('/api/session/start', (req, res) => {
  const { token, userName } = req.body;
  if (token !== '36') return res.status(401).json({ ok: false });
